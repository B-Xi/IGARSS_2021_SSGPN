...
def evaluate_preds(preds, labels, indices, epoch, NB_EPOCH, weight_dis=0):
...
def get_idx_train_val(labels, idx_train, num_classes, rate):
    ...
    for i in range(num_classes):
        ...
        val_idx.append(idx_c[idx[:round(num_c * rate)]])#random 10% for each class
    idx_val = np.concatenate(val_idx)
    mask[idx_val] = False
    return idx_train[(mask.tolist())], idx_train[idx_val]
